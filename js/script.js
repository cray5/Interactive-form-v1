...
$(document).ready(function () {
  ...
  $(".activities").append(
    `<br><span id="total-cost">Total: $${totalActivityCost}</span>`
  );
  ...
  $(".activities").on("change", function (event) {
    ...
    const $activityDateTime = $(isClicked).attr("data-day-and-time"); //getting the "data-day-and-time" attribute of the clicked checkbox and assigning it to the variable `$activityDateTime`.

    for (let i = 0; i < checkboxes.length; i++) {
      //looping through all the checkboxes which are stored in variable `checkboxes`
      ...
      if (
        $activityDateTime === $checkboxDateTime &&
        checkboxes[i] !== isClicked
      ) {
        if ($(isClicked).prop("checked")) {
          //if the property 'checked' is present on isClicked
          $(checkboxes[i]).attr("disabled", true); //disable all the other checkboxes with the same "data-day-and-time" value.
        } else {
          // if the property 'checked' is not present on isClicked
  ...
  nameInput.addEventListener(
    "input",
    createListener(isValidName, "#name-error")
  );
  ...
  emailInput.addEventListener(
    "input",
    createListener(isValidEmail, "#email-error")
  );
  //input listener to check and verify input simultaneously while user updates the text field.
  ...
  if ($("#other-title").attr("display") !== "none") {
    otherJobRoleInput.addEventListener(
      "input",
      createListener(isValidOtherJobRole, "#other-title-error")
    );
  }
  ...
  if ($("#credit-card").attr("display") !== "none") {
    creditCardNumInput.addEventListener(
      "input",
      createListener(isValidCCardNum, "#CardNum-error")
    );
    creditCardZipInput.addEventListener(
      "input",
      createListener(isValidCCardZip, "#zip-error")
    );
    creditCardCvvInput.addEventListener(
      "input",
      createListener(isValidCCardCvv, "#cvv-error")
    );
  }
